...
const setupEventListeners = (ctx) => {
  canvasRef.value.addEventListener('mousedown', startDrawing(e, ctx));
  canvasRef.value.addEventListener('mousemove', draw(e, ctx));
  canvasRef.value.addEventListener('mouseup', stopDrawing);
  canvasRef.value.addEventListener('mouseleave', stopDrawing);
}

// 开始绘画
const startDrawing = (e: MouseEvent, ctx) => {
