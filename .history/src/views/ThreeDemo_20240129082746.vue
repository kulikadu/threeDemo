...
onMounted(() => {
  ...
  if (threeDiv && renderer.domElement && WebGL.isWebGLAvailable()) {
    ...
    addEventListener('click', function (event) {
      ...
      //屏幕坐标px、py转NebGL标准设备坐标x、y
      //width、height表示canvas布宽尚度
      const x = (px / rendererWidth) * 2 - 1;
      const y = (py / rendererHeight) * 2 + 1;
      console.log('x:', x, 'y:', y);
      ...
      if (intersects.length > 0) {
        ...
        // const color = intersects[0].object.material.color.getHexString();
        intersects[0].object.material.color.set(new THREE.Color(0x00ff00));
