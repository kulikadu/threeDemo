...
onMounted(() => {
  ...
  const line = new THREE.LineSegments(geo, lineMaterial1);

  const points2 = [];
  points2.push(new THREE.Vector3(0, 0, 0));
  points2.push(new THREE.Vector3(1, 2, 3));
  const geo2 = new THREE.BufferGeometry().setFromPoints(points2);
  const line2 = new THREE.LineSegments(geo2, lineMaterial1);
  ...
  addEventListener('click', function (event) {
    ...
    const intersectsLine = raycaster.intersectObjects([line, line2]);
    ...
    if (intersectsWireframe.length > 0) {
      const faceIndex = intersectMesh[0].faceIndex; // 获取到被点击的面索引
      const verticesArray = []
      let vertexPositions = getVertex2(faceIndex, wireframe)
      for (let i = 0; i < vertexPositions.length; i++) {
        verticesArray.push(vertexPositions[i].x);
        verticesArray.push(vertexPositions[i].y);
        verticesArray.push(vertexPositions[i].z);
      ...
      const lineVertices = new Float32Array(verticesArray);
