...
onMounted(() => {
  ...
  for (let i = 0; i < boxGeo.index.count; i += 3) { // 每个面有三个索引
    // 添加面的第一个和第二个顶点之间的线
    edgeVertices.push(positionAttribute.getX(boxGeo.index.getX(i)), positionAttribute.getY(boxGeo.index.getX(i)), positionAttribute.getZ(boxGeo.index.getX(i)));
    edgeVertices.push(positionAttribute.getX(boxGeo.index.getX(i + 1)), positionAttribute.getY(boxGeo.index.getX(i + 1)), positionAttribute.getZ(boxGeo.index.getX(i + 1)));

    // 添加面的第二个和第三个顶点之间的线
    edgeVertices.push(positionAttribute.getX(boxGeo.index.getX(i + 1)), positionAttribute.getY(boxGeo.index.getX(i + 1)), positionAttribute.getZ(boxGeo.index.getX(i + 1)));
    edgeVertices.push(positionAttribute.getX(boxGeo.index.getX(i + 2)), positionAttribute.getY(boxGeo.index.getX(i + 2)), positionAttribute.getZ(boxGeo.index.getX(i + 2)));
  }
