...
onMounted(() => {
  ...
  addEventListener('click', function (event) {
    ...
    if (intersects.length > 0) {
      const faceIndex = intersects[0].faceIndex; // 获取到被点击的面索引
      const verticesArray = []
      let vertexPositions = getVertex(faceIndex, mesh)
      ...
      if (faceIndex % 2 == 0) {
        vertexPositions = getVertex(faceIndex + 1, mesh)
      } else {
        vertexPositions = getVertex(faceIndex - 1, mesh)
      }
  ...
  // 根据索引获取顶点位置
  function getVertex(faceIndex, mesh) {
