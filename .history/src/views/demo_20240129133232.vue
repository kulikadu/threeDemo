...
onMounted(() => {
  ...
  function onClick(event: any) {
    ...
    if (intersectsCube.length > 0) {
      ...
      const vertexIndex = cube.geometry.index.array.slice(faceIndex * 3, faceIndex * 3 + 3); // 获取三角形顶点索引
      ...
      const currentNormal = intersectsCube[0].normal;
      const verticesArray = []
      if (currentNormal.z == 1) {
        ...
        for (let i = 0; i < vertexPositions.length; i++) {
          if (vertexPositions[i].x != vertexPositions[i].y) {
            verticesArray.push(vertexPositions[1].y)
            verticesArray.push(vertexPositions[1].x)
            verticesArray.push(vertexPositions[1].z)
